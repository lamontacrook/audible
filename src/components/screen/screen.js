...
const Screen = () => {
  ...
  useEffect(() => {
    ...
    sdk.runPersistedQuery(`audible/${context.pqs.config}`, { path: configPath })
      .then(({ data }) => {
        if (data) {
          ...
          sdk.runPersistedQuery(`audible/${context.pqs.screen}`, params)
            .then(({ data }) => {
              if (data) {
  ...
  return (
    <React.Fragment>
      <Helmet>
        <title>Audible | {title}</title>
      </Helmet>
      {data && data.screen &&
        <div className='screen' {...editorProps(data.screen.body, `${title} Screen`, '', 'reference')} >
          <header><Navigation className='light' config={config} /></header>
